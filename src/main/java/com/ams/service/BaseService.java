...
public class BaseService {
    ...
    public static Map<String,List<String>> oaEFields = new HashMap<String, List<String>>(); //附件类型的对应表
    ...
    public void init() {
        ...
        List<PTable> etabList = baseDao.getEtabList(oaEfileMapping);
        ...
        for (PTable pTable : etabList) {
            efieldTemp = oaEFields.get(pTable.getF2());
            if(efieldTemp==null){
                efieldTemp = new ArrayList<String>();
            }
            efieldTemp.add(pTable.getF1());
            oaEFields.put(pTable.getF2(),efieldTemp);
            efieldTemp=null;
        }
    }
...
//		File newFile = new File(efilepath + File.separator + efile.getName());
    ...
    @Autowired
    @Value("${oa.efile.mapping}")
    protected String oaEfileMapping;//oa Efile mapping
    @Autowired
    @Value("${php.qzh}")
